...
|:---|:---|:---|:---:|:---:|
| **HU-01** | Como **cajero**, quiero registrar un cliente natural con sus datos personales para aperturar una cuenta en el sistema | - El sistema valida que el documento de identidad no esté repetido. El sistema lanza `CapacidadExcedidaException` si el banco alcanzó su límite de clientes. Los datos nulos o vacíos lanzan la excepcion `DatoInvalidoException` | alta | 5 |
| **HU-02** | Como **cajero**, quiero abrir una cuenta tanto natural como empresarial para que los clientes puedan utilizar el sistema bancario | El sistema rectifica la existencia del usuario, el sistema lanza la excepcion `CapacidadExcedidaException` si el cliente excede el numero maximo de cuentas (5) | alta | 4 |
| **HU-03** | como **cajero**, quiero realizar un deposito a una cuenta para asegurar el dinero del cliente en su cuenta de banco | El sistema valida el monto a depositar, el sistema valida si hay saldo disponible para el deposito y lanza `SaldoInsuficienteException` si no hay saldo suficiente, el sistema valida si la cuenta a depositar esta bloqueada y lanza la excepcion `CuentaBloqueadaException` si la cuenta esta bloqueada | alta | 5 |
| **HU-04** | como **cajero**, quiero realizar retiros para que los clientes tengan su dinero a disposición | El sistema valida el monto a retirar, el sistema valida si hay saldo suficiente para el retiro y lanza la excepcion `SaldoInsuficienteException` si no hay saldo suficiente, el sistema valida si la cuenta esta bloqueada y lanza la excepcion `CuentaBloqueadaException` si la cuenta esta bloqueada | alta | 4 |  
| **HU-05** | como **asesor financiero**, quiero realizar transferencias entre cuentas para brindar movimientos de montos seguros | El sistema valida el monto a transferir, el sistema valida si hay saldo suficiente y lanza la excepcion `SaldoInsuficienteException` si no hay saldo suficiente, El sistema valida si la cuenta a transferir esta bloqueada y lanza la excepcion `CuentaBloqueadaException` si la cuenta esta bloqueada | media | 4 |
| **HU-06** | como **asesor financiero**, quiero consultar la información de los clientes para brindar un soporte y atencion al cliente más personalizado | El sistema valida que el cliente esta registrado y lanza la excepcion `ClienteNoEncontradoException` si el cliente no esta registrado | baja | 2 |
| **HU-07** | como **asesor financiero**, quiero gestionar el estado de una cuenta para administrar el acceso a los servicios del banco | El sistema valida si la cuenta a gestionar existe | media | 3 |
| **HU-08** | como **gerente de sucursal**, quiero aprobar creditos para agilizar y/o autorizar el prestamo a los clientes | El sistema valida si el cliente esta registrado y lanza la excepcion `ClienteNoEncontradoException` si el cliente no esta registrado, el sistema valida si la persona que aprobara el credito tiene los permisos para hacerlo y lanza la excepcion `PermisoInsuficienteException` si no cuenta con los permisos requeridos | alta | 5 |
| **HU-09** | como **gerente financiero**, quiero consultar y calcular la nomina de los empleados para administrar adecuadamente sus salarios | El sistema valida si el empleado esta registrado, El sistema calcula la nomina del empleado | alta | 4 |
| **HU-10** | como **gerente financiero**, quiero registrar empleados para administrar de forma interna sus procesos financieros | El sistema valida que el documento del empleado no este repetido | alta | 5 |
| **HU-11** | como **cliente**, quiero recibir notificaciones para estar actualizado de los cambios que ocurren en mi/s cuenta/s | El sistema debe notificar al cliente si este permite recibir notificaciones | media | 3 |
| **HU-12** | como **sistema auditor**, quiero consultar el historial de transacciones para tener un control preciso de los movimientos en el banco | El sistema devuelve el historial de un cliente teniendo en cuenta el limite de transacciones a retornar (20 por cuenta) | media | 3 |
---
